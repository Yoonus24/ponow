...
class _PaymentDonePageState extends State<PaymentDonePage> {
  ...
  void _filterPayments() {
    ...
    final filtered = provider.payments.where((payment) {
      debugPrint("\nChecking Payment:");
      debugPrint("Invoice No: ${payment.invoiceNo}");
      ...
      debugPrint("Status Match: $matchesStatus");
      ...
      if (_fromDate != null && _toDate != null) {
        ...
        debugPrint("Original Payment Date: ${payment.paymentDate}");
        debugPrint("Converted Payment Date: $paymentDate");

        if (paymentDate == null) {
          debugPrint("❌ Skipped: paymentDate is null");
          return false;
        }
        ...
        if (paymentDate.isBefore(start) || paymentDate.isAfter(end)) {
          debugPrint("❌ Skipped: Outside date range");
          return false;
        } else {
          debugPrint("✅ Passed Date Filter");
        }
      }
      ...
      debugPrint("Search Match: $searchMatch");

      if (!searchMatch) {
        debugPrint("❌ Skipped: Search mismatch");
      } else {
        debugPrint("✅ Passed Search");
      }
