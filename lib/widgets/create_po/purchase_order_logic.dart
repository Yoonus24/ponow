...
import 'package:purchaseorders2/models/po/discount_model.dart';
...
class PurchaseOrderLogic {
  ...
  void cleanup() {
    ...
    try {
      ...
      throw Exception("VALUE_NOTIFIER_DISPOSE_ERROR");
    }
  }
  ...
  void applyLocationIfReady() {
    ...
    if (notifier.selectedLocation.isNotEmpty) {
      return;
    }
  ...
  void _clearVendorDetails() {
    ...
    try {
      ...
      throw Exception("VENDOR_DETAILS_CLEAR_ERROR");
    }
  }
  ...
  Future<void> savePurchaseOrder({
    ...
    if (vendorController.text.isEmpty || notifier.selectedVendor.isEmpty) {
    ...
    try {
      ...
      // HANDLE FAILURE HERE
      if (!success) {
        if (notifier.selectedLocation.isEmpty) {
          _showRequiredFieldSnackBar(
            'Please select location',
            scrollKey: vendorSectionKey,
          );
        } else if (notifier.selectedVendor.isEmpty) {
          _showRequiredFieldSnackBar(
            'Please select a vendor',
            scrollKey: vendorSectionKey,
          );
  ...
  bool _isControllerDisposed(TextEditingController controller) {
    try {
      ...
      throw Exception("TEXT_CONTROLLER_DISPOSE_ERROR");
    }
  }
  ...
  DateTime? parseDate(String dateString) {
    ...
    try {
      ...
      throw Exception("DATE_PARSE_ERROR");
    }
  }
